...
router.post("/", async (req, res, next) => {
  try {
    ...
    const extras = {};
    for (const k of ["vehicle", "national_id", "notes", "plate"]) if (k in req.body) extras[k] = req.body[k];
    let row;
    try {
...
router.patch("/:id", async (req, res, next) => {
  try {
    const patch = {};
    for (const k of ["name", "phone_number", "branch", "active", "vehicle", "national_id", "notes", "plate"]) if (k in req.body) patch[k] = req.body[k];
