...
- **Pre-flight checks must read where the data actually lives.** Two of mine were false alarms on every restaurant: the contact number is at `basic_info.contact.phone` (and `restaurants.phone_number`), not `basic_info.phone`/`hotline`; and the wpid is a **platform** fact — there is one WhatsApp number, exactly one restaurant holds it, so warning on everyone else told the founder to fix something already correct. A check that cries wolf is worse than no check.
- **`service_boots` skips local restarts** (`env === "local"`), or a laptop's restarts bury the real deploy history — 11 "deploys" in a week, most of them mine. `OPS_RECORD_LOCAL_BOOTS=1` opts back in.
- **The platform p95 is the worst tenant's p95**, not a true platform percentile — percentiles cannot be merged across databases. It is labelled that way in the UI; per-restaurant p95 (from the SQL functions) is exact.
- **Per-model / per-step money is sampled until migration 037.** While sampled, the console shows *shares*, never amounts: a 3% sample summed to $0.16 against a real $38 and read like a bug. With 037 the same table shows exact spend, and the sampling caveat disappears. `models_exact` / `steps_exact` on the response say which you are looking at, and exact mode is only used when **every** tenant has it — mixing an exact tenant with a sampled one gives a total that is neither.
