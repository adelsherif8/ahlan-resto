...
Last full green baseline: **91/91** — since then, targeted-verified additions (confirm in next full run): `stuckhandoff` (C2 re-ask circuit breaker), `optreassign` (set_option), zone pin-coords for share-location auto-quote. Suite now 92 cases.

Baseline was **91/91** (2026-08-11, all live-QA fixes: multi-intent, draft-merge, cancel-all draft-clear, set_option correction, bold names, accept-rec)
