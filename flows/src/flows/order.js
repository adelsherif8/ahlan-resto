...
Rules: qty defaults 1; ONLY names from MENU — return the name WITHOUT the (category); menu entries read "English name / Arabic name" — BOTH refer to the SAME dish and you ALWAYS return the ENGLISH name; match within the RIGHT category ("fried chicken" → a Chicken item, NEVER a beef burger); a MEAL's sides/drinks spoken with it ("iconic meal with fries and a cola") are that meal's choices — put them in that item's "notes", NEVER as separate items; an unclear/GARBLED word (voice notes!) is SKIPPED, never guessed — but a CLEARLY named product we don't carry ("a cola zero", "red bull") goes into items AS THE GUEST SAID IT (code reports it as unavailable — silently dropping it loses part of the order); if the RECENT CONVERSATION shows the guest already AGREED to a specific dish we recommended ("thats nice i want this" after we pitched it) and it is NOT already in the draft, include that dish in items too — to the guest it's ALREADY on their order and dropping it loses it ("also I want X" means X in ADDITION to it); never duplicate a dish the draft already has; a NEGATION ("I didn't order X", "مطلبتش X") is edits op "remove", never an item — BUT a bare "no" + item + OPTION word ("no, nashville slaw combo, the other is not") is NEVER removal: it's correcting WHICH item has that option → edits op "set_option" per item. DIRECTION MATTERS: the item NAMED WITH the option RECEIVES it — "no, nashville slaw combo, the other is not" = Nashville Slaw GETS combo ([{op:"set_option", item:"Nashville Slaw", option:"combo"}]) and "the other" (the other draft item) gets the opposite ({op:"set_option", item:"<the other item>", option:"sandwich"}); NEVER swap them; removal needs remove-words (remove/take off/شيل/didn't order); an instruction about ONE item ("burger without onion") belongs in that item's "notes", NOT the order-level "notes"; "edits" is for CHANGING an order being built — "add a coke"/"زود كوكاكولا"/"zawed coke" → op add, "remove the fries"/"شيل البطاطس"/"sheel el batates" → op remove, "make it 2"/"خليهم ٢"/"khaleehom etnen"/"actually just one" → op set_qty with qty, "change the drink to fanta"/"غيّر المشروب فانتا"/"ghayar el mashroob fanta" (a drink/size/format CHOICE of an item already on the order) → op set_option with option = the choice (never a replace of dishes), "no I want the chicken ranch instead"/"actually give me X"/"change it to X"/"مش عايز كذا, عايز X"/"بدل ده هاتلي X" (naming a DIFFERENT menu item to SWAP for one already on the order, mid-question or not) → op replace with "item" = the item being swapped OUT (closest MENU name; omit/null if only one item is on the order so it's unambiguous) and "with" = the item being swapped IN (when they change something, use edits and leave "items" null); "cancel_order" = wants to cancel/scrap the WHOLE order or start over ("cancel it all", "cancel this order", "start over", "forget the whole thing", "الغي الاوردر", "بلاش كله", "من الأول") — removing ONE item is edits op "remove", never cancel_order; "status" = asking where their order is; "confirm" = agreeing to place the order we just summarised (yes/confirm/تمام/اوكي/go ahead); "repeat_last" = wants their usual / same as last time ("same as last time", "the usual", "نفس الطلب", "زي كل مرة", "nafs el order") — items stay null, we rebuild from their history; "question" = the guest is ASKING about the restaurant, not ordering — delivery coverage or fee ("do you deliver to Maadi?", "بتوصلوا المعادي؟ وبكام", "بتوصلوا لحد فين"), hours, ingredients, availability, "do you have tissues" — anything you'd ANSWER rather than put in a cart, even mid-order. CRITICAL: a bare ANSWER to a question WE asked — a size ("Medium"), a drink ("Sprite"), "cash"/"card", a branch name, an address, "yes" — is NEVER "question"; only a real interrogative about the place is.`;
    ...
    // "غيّر المشروب فانتا" came back as op "remove Soft Drink" (and once as an item). A
    // CHANGE verb followed by a word that IS a choice on a draft line is set_option,
    // whatever the model called it — code checks the word against the real choices.
    if (loaded.pending?.items?.length && !(e.edits || []).some((ed) => ed?.op === "set_option")) {
      const msgL2 = String(input.message || "").toLowerCase().trim();
      const m2 = /(?:^|\s)(?:ghayy?ar|change|switch|badd?el|غيّر|غير|بدّل|بدل)\s+(?:el\s+|the\s+|ال)?([a-z\p{L}][a-z\p{L} ]{2,20}?)\s+(?:to\s+|le\s+|لـ|ل)?([a-z\p{L}][a-z\p{L} ]{2,25})$/iu.exec(msgL2);
      if (m2 && msgL2.length <= 60) {
        const want = normName(arOptionWords(m2[2].trim()));
        const w0 = want.split(" ")[0] || "";
        const fits = loaded.pending.items.some((it) => (it.option_defs || []).some((g) => groupChoices(g, loaded.menu).some((c) => {
          const cn = normName(c.name); const c0 = cn.split(" ")[0] || "";
          return cn === want || (w0.length >= 4 && c0 === w0) || (want.includes("cola") && cn.includes("cola") && !cn.includes("diet"));
        })));
        if (fits) { e.edits = [{ op: "set_option", item: loaded.pending.items.length === 1 ? null : (loaded.pending.items[0]?.name || null), option: m2[2].trim() }]; e.items = null; }
      }
    }
    ...
    const outcome = await f.node("act", async () => {
      ...
      // A bare word that is one of a draft item's OPTION CHOICES ("medium", "hot",
      // "combo", «وسط») is an answer, even if the question on the table was the order
      // TYPE (type-first asks that before the options): the guest is volunteering the
      // next answer, not chatting. It used to fall to friendly, which invented
      // "Medium Fries are 60 EGP".
      ...
      if (e.intent === "repeat_last") {
        ...
        if (amb0?.candidates?.length) {
          ...
          // ARABIC answer («ناشفيل») against the candidates' Arabic menu names — the
          // distinguishing Arabic token(s) pick the dish, same as the English path
          if (!pick.length && /[؀-ۿ]/.test(input.message)) {
            const arNz2 = (s2) => String(s2 || "").replace(/[ً-ْـ]/g, "").replace(/[أإآ]/g, "ا").replace(/ة/g, "ه").replace(/ى/g, "ي");
            const saidAr = ` ${arNz2(input.message).replace(/[^؀-ۿ\s]/g, " ").replace(/\s+/g, " ").trim()} `;
            const arNameOfC = (c) => arNz2(loaded.menu.find((m) => normName(m.name) === normName(c))?.name_ar || "");
            const toks = (s2) => s2.split(" ").filter((w) => w.length >= 3 && !/^[٠-٩]+$/.test(w));
            const shared = new Set(amb0.candidates.map((c) => toks(arNameOfC(c))).reduce((a, b) => a.filter((x) => b.includes(x))));
            const scored = amb0.candidates.map((c) => ({ c, hits: toks(arNameOfC(c)).filter((w) => !shared.has(w) && saidAr.includes(` ${w} `)).length }));
            const best = Math.max(0, ...scored.map((s2) => s2.hits));
            if (best > 0) { const top = scored.filter((s2) => s2.hits === best); if (top.length === 1) pick = [top[0].c]; }
          }
          ...
          if (pick.length === 1) {
            ...
            // the model's "note" is often just the guest's own pick word ("chocolate")
            // — a note that names (part of) the picked dish is no note; kept, it became
            // a phantom which-one for "chocolate" right after the pick
            const pn0 = normName(e.items?.[0]?.notes || "");
            const noteOk = pn0 && !normName(pick[0]).includes(pn0) && !amb0.candidates.some((c) => normName(c).includes(pn0));
            e.items = [{ name: pick[0], qty, notes: noteOk ? e.items[0].notes : null }, ...others];
        ...
        // OUR text (verifiable), never by trusting the model harder.
        {
          ...
          // "great, one of those as a sandwich and a classic burger for me, pickup, cash,
          // now" — the referent is the FIRST clause; a long message that OPENS with an
          // anaphora is judged on that clause, not skipped for its length (skipping let
          // the model's guess — J'S Special Burger for a no-beef daughter — stand)
          const wholeShort = input.message.trim().split(/\s+/).filter(Boolean).length <= 6;
          const firstClause0 = input.message.trim().split(/\s*(?:,|،|\band\b|\bw\b|\bو\b|\bplus\b|\bكمان\b)\s*/iu)[0] || input.message.trim();
          const ANAPH0 = /^\W*(?:ok(?:ay)?|great|tamam|تمام|طيب|ماشي|yes|aywa|ايوه|أيوة)?\W*(?:add|get me|give me|i want|i ll take|ill take|hat|hatli|zawed|zawed li|عايز|هات|هاتلي|زود|زوّد|ضيف|زودلي)?\W*(?:one|1|واحد|واحدة|wa7ed|wa7da|it|that|this|those|these|ده|دي|دول|da|di|dol)(?![\p{L}\p{N}])/iu;
          const clauseShort = !wholeShort && ANAPH0.test(firstClause0) && firstClause0.split(/\s+/).filter(Boolean).length <= 8;
          const shortReply = wholeShort || clauseShort;
          // what we judge the referent on: the whole (short) message, or just its opening clause
          const refText = wholeShort ? input.message : firstClause0;
          if (lastAi && shortReply && !amb0?.candidates?.length && !loaded.pending?.awaiting_option) {
            ...
            const typedFull = (e.items || []).some((it) => { const n = normName(it.name); return n && normName(arOptionWords(refText)).includes(n); });
            ...
            if (isAnaphora) {
              ...
              // «اللودد فرايز» matches BOTH "Loaded Fries" and plain "Fries" (the Arabic
              // word maps to "fries") — the longer, more specific name is what they asked
              // about; a name contained in another match is dropped
              const askedSpecific = askedAbout.filter((m) => !askedAbout.some((o) => o !== m && normName(o.name).includes(normName(m.name)) && normName(o.name) !== normName(m.name)));
              if (askedSpecific.length === 1) { offered.length = 0; offered.push(askedSpecific[0]); }
            }
            // the COUNT is the guest's word in the clause ("add one" = 1, "two of those" = 2),
            // never the model's qty — it wrote 2 for "ok add one" (one MORE Classic Burger,
            // in its reading) and two Loaded Fries landed on the bill
            const anaphQty = (() => {
              const NUMW = { one: 1, two: 2, three: 3, four: 4, wa7ed: 1, wa7da: 1, etnen: 2, etneen: 2, talata: 3, tlata: 3, arba3a: 4, "واحد": 1, "واحدة": 1, "اتنين": 2, "إتنين": 2, "تلاتة": 3, "ثلاثة": 3, "اربعة": 4, "أربعة": 4 };
              const mq = /(?:^|[^\p{L}\p{N}])(\d{1,2}|[٠-٩]{1,2}|one|two|three|four|wa7ed|wa7da|etne+n|t?lata|arba3a|واحدة?|ا?تنين|إتنين|تلاتة|ثلاثة|ا?ربعة|أربعة)(?![\p{L}\p{N}])/iu.exec(firstClause);
              if (!mq) return 1;
              const raw = mq[1].replace(/[٠-٩]/g, (d) => String("٠١٢٣٤٥٦٧٨٩".indexOf(d)));
              const n = NUMW[raw.toLowerCase()] ?? Number(raw);
            ...
            if (offered.length >= 2) {
              const said = normName(arOptionWords(refText));
              const saidAr = arNz(refText);
              ...
              if (unique) {
                const qty = isAnaphora ? anaphQty : Math.min(Math.max(Math.round(Number(e.items?.[0]?.qty || e.edits?.[0]?.qty) || 1), 1), 20);
                ...
                // reply to our list carries no other dish; a longer message keeps the
                // dishes the guest NAMED in its later clauses
                const msgN2 = normName(arOptionWords(input.message));
                const extras = wholeShort ? [] : (e.items || []).filter((it) => { const n = normName(it.name); return n && n !== normName(best.m.name) && (msgN2.includes(n) || n.split(" ").filter((w) => w.length >= 4).every((w) => msgN2.includes(w))); });
                e.items = [{ name: best.m.name, qty, notes }, ...extras];
                if (Array.isArray(e.edits)) e.edits = e.edits.filter((ed) => ed?.op !== "add" && ed?.op !== "replace");
        ...
        // THE GUEST'S NUMBER WINS. "3 tenders please" came back qty 1 — the model read
        // the 3 as the dish's own "3 Pcs" (and once as a note "(3 pieces)"). When the
        // message carries a leading count right before the dish words and the model
        // says 1, code restores the guest's count. Only for a single-dish message (a
        // multi-item message keeps the model's per-item numbers).
        if ((e.items || []).length === 1 && (Number(e.items[0].qty) || 1) === 1) {
          ...
          if (m) {
            ...
            // — a 4x4 bundle is ONE bundle, not four
            // …but only when the guest TYPED the dish's own numbered name ("4x4",
            // "tenders 3 pcs") — a leading "3" before "tenders" is a count of three,
            // even though the dish is "Tenders 3 Pcs"
            ...
            const numberedCore = dishNumIdx >= 0
              ? dishN.split(" ").filter((w, i, arr) => { const ni = arr.findIndex((x) => x === String(n)); return ni >= 0 && Math.abs(i - ni) <= 1; }).join(" ")
              : "";
            const guestTypedDishNumber = !!numberedCore && numberedCore.includes(String(n)) && msgNorm.includes(` ${numberedCore} `);
            if (Number.isFinite(n) && n >= 2 && n <= 20 && !sizeWord && !guestTypedDishNumber) {
              e.items[0].qty = n;
              if (e.items[0].notes && /^\s*\d+\s*(pcs?|pieces?)\s*$/i.test(e.items[0].notes)) e.items[0].notes = null; // "(3 pieces)" was the count, not a note
            }
          }
        }
        // MID-ORDER, a REMOVAL in the message, and a NEW dish the guest never named:
        // that's the model reading the removed ingredient as a dish — "sandwich we men
        // gheir mashroom" came back with a Truffle SHROOM Burger added and the "no
        // mushroom" pinned to it. The dish is dropped; the removal lands on the draft.
        if (loaded.pending?.items?.length && Array.isArray(e.items) && e.items.length && !answeredAmbiguity
          ...
          e.items = e.items.filter((w) => {
            ...
            if (!n) return true;
            if (loaded.pending.items.some((it) => normName(it.name) === n)) return true;   // already on the draft — an edit, kept
            const row = loaded.menu.find((m) => normName(m.name) === n) || loaded.menu.find((m) => normName(m.name).includes(n));
            if (!row) return true;                                                          // unknown-dish path owns it
            const en = normName(row.name);
            const toks = en.split(" ").filter((t) => t.length >= 4 && !GENERIC.has(t));
            // whole tokens only — "shroom" sits inside "mashroom" (the removed ingredient!)
            const enOk = saidN.includes(en) || toks.some((t) => ` ${saidN} `.includes(` ${t} `) || (t.length >= 5 && saidN.split(" ").some((x) => editDistance(x, t) <= 1)));
            const ar = String(row.name_ar || "").replace(/[ً-ْـ]/g, "").replace(/[أإآ]/g, "ا").replace(/ة/g, "ه").replace(/ى/g, "ي");
            const arOk = !!ar && ar.split(/\s+/).filter((t) => t.length >= 3 && !/^[٠-٩]+$/.test(t)).some((t) => saidAr.includes(t));
        ...
        for (const w of wanted) {
          ...
          // a fragment pinning exactly one dish ("caramelised burger") still auto-takes.
          {
            ...
            if (!msgN2.includes(hitN) && !namedInArabic && !weOfferedIt && hitN !== normName(answeredAmbiguity || "")) {
              ...
              if (fragToks.length) {
                const fragHits = useAr
                  ? loaded.menu.filter((m) => m.name_ar && fragToks.every((tk) => arToksOf(m.name_ar).includes(tk)))
                  : loaded.menu.filter((m) => { const n2 = normName(m.name); return fragToks.every((t) => n2.includes(t)); });
        ...
        for (const ed of (e.edits || []).slice(0, 6)) {
          ...
          if (ed.op === "add") {
            ...
            let idx = items.findIndex((it) => normName(it.name).includes(target) || target.includes(normName(it.name)));
            // set_option naming the CHOICE's own product ("Soft Drink" for «غيّر المشروب
            // فانتا») — the target is the draft line whose choices carry that option,
            // not a dish called Soft Drink (which isn't on the order → "couldn't find")
            if (idx < 0 && ed.op === "set_option" && ed.option) {
              const want = normName(arOptionWords(String(ed.option)));
              const w0 = want.split(" ")[0] || "";
              idx = items.findIndex((it) => (it.option_defs || []).some((g) => g.key !== "slots" && groupChoices(g, loaded.menu).some((c) => { const cn = normName(c.name); return cn === want || (w0.length >= 4 && cn.split(" ")[0] === w0) || (want.includes("cola") && cn.includes("cola") && !cn.includes("diet")); })));
            }
      ...
      // A LANDMARK the restaurant listed, written in a delivery message, IS the address —
      // even when the model filed it under order notes ("delivery point 90" for
      ...
      // resolving a typed address to a point only matters when the restaurant USES the
      // point: drawn/circle zones or distance pricing. A restaurant with neither
      // (Just Smash: flat fee, no zones) keeps the address as text on the ticket —
      // no geocoder, no "which place?" for a place it doesn't need to locate.
      const usesPoint = !!((config.delivery?.zones || []).length || branches.some((b) => (b.delivery_zones || []).length) || pricingOf(config).mode === "distance");
      if (pickedPlace) geoPoint = pickedPlace;
      else if (usesPoint && orderType === "delivery" && address && mapLink?.lat == null && !sharedPin) {
      ...
      const isChoiceTalk = (it, nt) => {
        ...
        const hits = words.filter((w) => choiceWords.has(w)).length;
        return hits >= Math.ceil(words.length * 0.6);
      };
      for (const it of items) {
        const nt = String(it.notes || "").trim();
        if (!nt || MODIFIER_SIG.test(nt) || isChoiceTalk(it, nt)) continue;
      ...
      for (const it of items) {
        ...
        const queue = String(it.notes).split(/\s*[,·،]\s*/).filter(Boolean);
        const keep = [];
        for (let qi = 0; qi < queue.length && qi < 12; qi++) {
          ...
          for (const g of it.option_defs || []) {
            ...
            // COMPOUND note — "sandwich no pickles" / "as a sandwich" / "combo with cola":
            // exactly one choice word inside it → that choice is set; what's left (minus
            // filler) stays a note ("no pickles") or is re-read for the next group
            // ("with cola" → the combo drink). It used to sit whole as a note and the
            // choice was asked again — twice — after the guest had already said it.
            // (a COUNT before the choice — "1 mild", "2 hot" — is a per-unit split for the
            // resolver, never filler: promoting "1 mild" set Mild on all three tenders)
            const lead = pn.replace(/^(?:(?:as an?|as|the|a|an|make it|please)\s+)+/, "");
            if (/^\d/.test(lead)) { keep.push(part); used = true; break; }
            // …and never when the WHOLE note splits units of this group ("hot for the
            // first 2, mild for the third" — two choices across the parts, or a per-unit
            // marker in the remainder): the resolver's splitter owns that note
            const wholeN = normName(arOptionWords(String(it.notes)));
            const groupSplit = Number(it.qty) > 1 && chs.filter((o) => { const f0 = normName(o.name).split(" ")[0]; return f0.length >= 3 && new RegExp(`(^|\\s)${f0}(\\s|$)`).test(wholeN); }).length > 1;
            const restHasUnit = /(^|\s)(for|first|second|third|fourth|rest|last|other|others|\d+)(\s|$)/.test(pn.replace(/^(?:(?:as an?|as|the|a|an|make it|please)\s+)+/, "").split(" ").slice(1).join(" "));
            if (named.length === 1 && !groupSplit && !restHasUnit && (lead === normName(named[0].name).split(" ")[0] || lead.startsWith(`${normName(named[0].name).split(" ")[0]} `))) {
              const f = normName(named[0].name).split(" ")[0];
              it.options = { ...(it.options || {}), [g.key]: named[0].name };
              const rest = pn.replace(new RegExp(`(^|\\s)${f}(\\s|$)`), " ").replace(/(^|\s)(as an?|as|the|a|an|make it|please|with|and|w|el)(?=\s|$)/g, " ").replace(/\s+/g, " ").trim();
              if (rest.length >= 3) queue.push(rest);
              used = true; break;
            }
          }
      ...
      for (let pass = 0; pass < 2 && step.ask && guestSaidSomething; pass++) {
        ...
        // CODE FIRST for a per-unit split on a multi-unit line: "hot for the first 2
        // and mild for the third" — the model gave 3 per-unit answers one run and a
        // single "Hot" the next. Deterministic: find every choice name in the message
        // with its unit count (positional/count words), and if they add up to the
        // line's qty with more than one distinct choice, split the line right here.
        if (pass === 0 && openGroups.length === 1 && Number(askedItem.qty) > 1) {
          const g0 = openGroups[0];
          const saidN = normName(arOptionWords(input.message));
          // locate every choice mention, then judge each one's count ONLY within its own
          // segment (up to the next mention) — "hot for the first 2 | and mild for the
          // third": the "2" belongs to hot, the ordinal to mild
          const mentions = [];
          for (const c of groupChoices(g0, loaded.menu)) {
            const cn = normName(c.name); if (!cn) continue;
            const key = cn.split(" ")[0]; if (key.length < 3) continue;
            const idxP = ` ${saidN} `.indexOf(` ${key} `);
            if (idxP >= 0) mentions.push({ name: c.name, at: idxP, len: key.length });
          }
          mentions.sort((a, b) => a.at - b.at);
          const picked = [];
          mentions.forEach((m, i) => {
            // a mention OWNS the gap before it (back to the previous mention's segment
            // end — for "2 hot and 1 mild" the "1" sits there) and the text after it up
            // to the next mention ("hot for the first 2 and …")
            const prevEnd = i > 0 ? mentions[i - 1].at + mentions[i - 1].len : 0;
            const nextAt = i + 1 < mentions.length ? mentions[i + 1].at : saidN.length;
            // the previous mention's own trailing count ("first 2") must not leak into
            // this one: keep only the tail of the gap after any earlier count/ordinal
            // only a count that sits IMMEDIATELY before this mention belongs to it
            // ("2 hot", "one mild", "the last one mild"); everything earlier in the gap
            // is the previous mention's tail
            const gapAll = saidN.slice(prevEnd, m.at);
            // "…for the first two, medium…" — that "two" completes the PREVIOUS name's
            // "first two", it is not medium's count: a count right after "first" binds
            // backwards. Only a bare count ("2 hot", "one mild") or "the last one" leads.
            const lead = i === 0
              // nothing before the first name can belong to anyone else — take it all ("first two hot")
              ? /(?:(?:the\s+)?(?:first|last|other)\s+)?(?:\d{1,2}|one|two|three|four|five|six)\s*$/i.exec(gapAll)
              : /(?:(?:the\s+)?(?:last|other)\s+one|(?<!(?:first|last|other)\s{1,3})(?:\d{1,2}|one|two|three|four|five|six))\s*$/i.exec(gapAll);
            const gapBefore = lead ? lead[0] : "";
            const seg = `${gapBefore}${saidN.slice(m.at, nextAt)}`;
            const q = unitsForChoice(seg, gapBefore.length, m.len) ?? 1;
            for (let k = 0; k < q; k++) picked.push(m.name);
          });
          if (new Set(picked).size > 1 && picked.length <= Number(askedItem.qty)) {
            const perUnit = [...picked];
            while (perUnit.length < Number(askedItem.qty)) perUnit.push(perUnit[perUnit.length - 1]);
            const counts = {}; for (const n of perUnit) counts[n] = (counts[n] || 0) + 1;
            const lines = Object.entries(counts).map(([n, q]) => ({ ...askedItem, qty: q, options: { ...askedItem.options, [g0.key]: n } }));
            step.items.splice(step.ask.index, 1, ...lines);
            items = step.items;
            step = nextQuestion(items, loaded.menu, "", loaded.pending, currency);
            if (!step.ask) break;
            continue; // re-evaluate against the next open ask (pass 1)
          }
        }
...
RULES: only exact strings from the lists; null when the message doesn't clearly answer that question; FORMAT questions (sandwich-vs-combo/meal): the guest describing the SERVING STYLE answers it — a bare "burger"/"just the burger"/"just a sandwich"/"لوحده"/"بس ساندوتش"/"not a combo"/"من غير وجبة" → the sandwich-only choice (even when the dish name contains "burger"); "combo"/"meal"/"full meal"/"وجبة"/"كومبو"/"with fries and a drink/cola" → the combo/meal choice (and any named drink/side in the same breath answers those questions too); BUT the dish's FULL name alone when ORDERING ("iconic meal" as a new order) does not choose a format; a variant/brand we don't carry maps to its closest REAL EQUIVALENT in the list — like a sharp cashier: "cola zero"/"coke zero" → the diet/sugar-free cola in the list; "pepsi" → the cola we carry; "7up" → "Sprite"; "بيبسي دايت" → the diet cola — put the LIST string in answers AND report it in "equivalent". "used" and every answer MUST be an EXACT string from the lists — NEVER a product that isn't listed (never invent "Coca-Cola Zero" when the list has "Coca - Cola Diet"). Only when NOTHING in the list is a reasonable equivalent (e.g. they asked for a milkshake and the list is sodas): answers null + "unmatched". Equivalence is about the same KIND of thing — never map across kinds; never invent. The examples in these instructions ("V Cola", "Diablo fries", "7up"→"Sprite") are ILLUSTRATIONS — never output any string that is not literally in THIS request's lists.`;
          ...
          // PER-UNIT ANSWERS: one open group, a multi-unit line, and the model returned
          // one answer PER UNIT ("hot for the first 2 and mild for the third" →
          // {1:Hot, 2:Hot, 3:Mild}). Keying those by group index applied "Hot" to all
          // three. When they're all valid choices of that group and not all the same,
          // split the line like a real ticket (2× Hot, 1× Mild).
          if (openGroups.length === 1 && Number(askedItem.qty) > 1 && !askedItem.options[openGroups[0].key]) {
            const g0 = openGroups[0];
            const vals = Object.keys(answers).sort((a, b) => Number(a) - Number(b)).map((k) => answers[k]).filter((v) => typeof v === "string" && v.trim());
            const choices = groupChoices(g0, loaded.menu);
            const canon = vals.map((v) => choices.find((c) => c.name === v.trim() || normName(c.name) === normName(v))?.name || null);
            if (vals.length >= 2 && vals.length <= Number(askedItem.qty) && canon.every(Boolean) && new Set(canon).size > 1) {
              const perUnit = [...canon];
              while (perUnit.length < Number(askedItem.qty)) perUnit.push(perUnit[perUnit.length - 1]); // "hot for the first 2, mild for the rest"
              const counts = {};
              for (const n of perUnit) counts[n] = (counts[n] || 0) + 1;
              const idx = step.items.indexOf(askedItem);
              const lines = Object.entries(counts).map(([n, q]) => ({ ...askedItem, qty: q, options: { ...askedItem.options, [g0.key]: n } }));
              if (idx >= 0) { step.items.splice(idx, 1, ...lines); appliedAny = true; lines.forEach((l) => appliedVals.add(l.options[g0.key])); }
            }
          }
          if (!appliedAny) openGroups.forEach((g, i) => {
      ...
      // picks a dish, it doesn't strip the smoke.
      {
        ...
        if (target && !isQuestion) {
          ...
          while ((m2 = RM.exec(String(input.message || "")))) {
            ...
            // "no I want the truck meal" / "no, give me X" — a correction, not a removal:
            // a weak marker followed by an ORDERING verb is the guest changing their mind
            if (weakMarker && /^(i want|i d like|i'd like|i would like|give me|get me|make it|i meant|actually|3ayez|3ayza|3awez|hat|hatli|عايز|عايزة|عاوز|هات|هاتلي|خلي|اقصد|أقصد|قصدي)\b/iu.test(what)) continue;
      ...
      const pickUpsell = () => {
        ...
        if (!picks.length && up.auto !== false) {
          ...
          // the restaurant's own pairing (pairs_with) is offered regardless — they chose
          // it for that dish; the "no side on a combo" rule only shapes the AUTO picks
          const seedIsSide = seed && SIDE.test(`${seed.category || ""} ${seed.name}`);
          const seedIsDessert = seed && DESSERT.test(`${seed.category || ""} ${seed.name}`);
          const seedIsDrink = seed && DRINK.test(`${seed.category || ""} ${seed.name}`);
          const sidePick = seedIsSide ? seed : (!hasSide ? best(SIDE) : null);
          const dessertPick = seedIsDessert ? seed : (!hasDessert ? best(DESSERT) : null);
          const drinkPick = seedIsDrink ? seed : (!hasDrink ? best(DRINK) : null);
          picks = [sidePick, dessertPick, drinkPick].filter(Boolean).slice(0, 3);
          if (seed && !picks.includes(seed)) picks = [seed, ...picks].slice(0, 3);
...
LANGUAGE (last line so everything above stays cacheable): mirror the guest's language & script — ${lang}.`;
    ...
    // never on a menu request — the answer IS the menu; a stitched friendly line above
    // it ("تحت أمرك — تحب تعرف إيه؟") read as the bot not understanding
    // "cash, Levels Mall" came back with question:"Levels Mall" — the ADDRESS, not a
    // question — and friendly invented "we don't deliver to Levels Mall" on top of a
    // correctly priced bill. A "question" with no question mark / question word, or
    // one that is the address we just took (or a listed landmark), is not asked.
...
function nextQuestion(items, menu, message, pending, currency = "EGP") {
  ...
  // notes that only restate applied choices are ticket noise — drop them. Order-type
  // and payment words that the extractor swept into the note ("pickup, full meal,
  // medium…") are noise too — they live in their own fields.
  for (const it2 of out) {
    if (!it2.notes) continue;
    const chosen = normName(Object.values(it2.options).filter((v) => typeof v === "string").join(" "));
    const FILLER = new Set(["and", "with", "a", "an", "the", "please", "و", "مع", "pickup", "pick", "up", "delivery", "dine", "in", "takeaway", "cash", "card", "visa", "instapay", "for", "from", "to"]);
